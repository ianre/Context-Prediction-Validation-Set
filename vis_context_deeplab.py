...
class Iterator:
    ...
    :return: '''     
    #!LG_Info, RG_Info, LG_Thread_Info, RG_Thread_Info = self.DrawSingleImageContextKT(imageSource,labelSource,outputDest, MPI, CtxI,CtxI_Pred)
    def DrawSingleImageContextKT(self, grasperSource, ThreadSource, target,CtxI,CtxI_Pred, DEBUG=False):       
        #J = JSONInterface(labelSource)
        prednpy = NPYInterface2()
        (x_p,y_p), (i,j), inter = prednpy.getIntersection(grasperSource, ThreadSource) # (center), (intersection), boolean
        print((x_p,y_p), inter)
